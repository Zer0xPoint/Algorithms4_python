class DoubleNode:
    ...
    def delete_node(self, key):
        ...
        # elif self.__last.prior is None:
        #     self.delete_tail_node()
        # else:
        #     __current_node = self.find_node(key)
        #     if __current_node.next is not None:
        #         __current_node.next.prior = __current_node.prior
        #     if __current_node.prior is not None:
        #         __current_node.prior.next = __current_node.next
        #     self.__N -= 1
        __current_node = self.find_node(key)
        if __current_node.next is None:
            self.delete_tail_node()
        elif __current_node.prior is None:
            self.delete_head_node()
        else:
            __current_node.next.prior = __current_node.prior
            __current_node.prior.next = __current_node.next
