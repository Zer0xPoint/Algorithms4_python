...
def two_sum_faster(list_a):
    ...
    for i in range(len(list_a)):
        if dict_a.__contains__(-list_a[i]) and i != dict_a.get(-list_a[i]):
            # dict_a.pop(-matrix_a[i])
            # print(matrix_a[i], dict_a.get(-matrix_a[i]), i)
            # del dict_a[-matrix_a[i]]
            count += 1
        dict_a[list_a[i]] = i
    return count
...
def three_sum_faster(list_a):
    ...
    for i in range(len(list_a)):
        for j in range(i):
            if dict_a.__contains__(-(list_a[i] + list_a[j])) and i < dict_a.get(-(list_a[i] + list_a[j])):
                count += 1
    return count


a = [random.randint(-100, 100) for i in range(10000000000)]
print(two_sum(a))
print(three_sum(a))
